...
def get_images(endpoint):
    ...
    try:
        ...
        # Parse HTML
        page = Soup(s.page_source, "html.parser")
    ...
    for raw_link in raw_links:
        ...
        try:
            link = raw_link.span.text.split("\n")  # read from span tag
            link2= list(filter(lambda a: a != "", link))
            #link= (" ").join(link)
        except:
            continue
        # Display links
        file = open("Select Box Text.txt", "a")
        file.write(endpoint + "|" + bellpage + "|" + str(link2) + "\n")
        file.close()
...
if(__name__ == "__main__"):
    # Read links
    data = read_from_txt("SelectBox_Sites.txt")

    # Test
    #data = 'https://my2.siteimprove.com/Inspector/624660/Accessibility/Page?pageId=14507940436&impmd=6508FEDFC411F126AA9F07EE75D47280#/Criterion/1.3.1/Check/8'
    #get_images(data)
