...
def get_data(endpoint):
    ...
    try:  # In case of errors such as timeout/page not found
        ...
        anchor2 = Soup(anchor, "html.parser").findAll("a")  # Parse all anchor
    except:
    ...
    for anchor in anchor2:
        for img in anchor.findAll("img"):  # Find all img tags
            if not img.has_attr("alt") or img["alt"].strip() == "":  # find if alt attribute is empty or non existent
                file = open("Alt anchor.txt", "a")
                file.write(endpoint + "," + bellpage + "," + str(img) + "\n")
                file.close()
    s.quit()
...
if(__name__ == "__main__"):
    # Read links
    data = read_from_txt("Alt anchor sites.txt")
    # Test
    # data = 'https://my2.siteimprove.com/Inspector/624660/Accessibility/Page?pageId=17930156412&impmd=6508FEDFC411F126AA9F07EE75D47280#/Criterion/2.4.4/Check/5'
    # get_data(data)
